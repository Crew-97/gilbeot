...
export function getCards(state, condition = {}) {
  ...
  // 잠긴 카드는 제목과 본문(title, reason, sttText)을 응답에서 아예 제거한다 — 화면이 아니라 데이터에서 막는다 (해소 33)
  return result.map((c) => {
    const isUnlocked =
      c.authorDriverId === state.currentDriverId ||
      state.unlocks.some((u) => u.driverId === state.currentDriverId && u.cardId === c.id)
    if (isUnlocked) return { ...c, isUnlocked }
    return { ...c, title: '', reason: '', sttText: '', isUnlocked }
  })
}
...
export function addCard(state, aiCard, context) {
  ...
  // similar와 conflict 관계만 저장한다. similar일 때만 대상 카드의 교차 검증 횟수를 올린다
...
// 로그인 처리 (Mock, 해소 34). 현재 기사만 바꾼다. 실제 인증은 만들지 않는다
export function signIn(state, driverId) {
  const exists = Array.isArray(state.drivers) && state.drivers.some((d) => d.id === driverId)
  if (!exists) return { state, ok: false, reason: '기사를 찾을 수 없어요' }
  return { state: { ...state, currentDriverId: driverId }, ok: true, reason: null }
}

// 가입 처리 + signup_bonus (최초 1회)
export function signUp(state, driverId) {
  const alreadySignedUp = state.pointTransactions.some(
    (transaction) => transaction.type === 'signup_bonus' && transaction.driverId === driverId
  )
  if (alreadySignedUp) {
    return { state, ok: true, reason: null, alreadySignedUp: true }
  }
...
export function applyAdminAction(state, cardId, action) {
  ...
  const cards = state.cards.map((c) => {
    if (c.id !== cardId) return c
    if (action === 'keep') return { ...c, status: 'published' }
...
// ── AI 인터뷰 연동 (S-06 · /api/structure) ─────────────────────────

// /api/structure 요청 본문을 조립한다. API 는 순수 변환 함수라 조회를 하지 않는다 — 조립은 store 몫
// existingCards: 같은 장소의 기존 published 카드 (AI 유사도 판단용 — 화면 노출용이 아니다)
// sessionCards: 이번 인터뷰에서 이미 만든 카드
export function buildStructureRequest(state, interviewId, sttText, questionText, isFollowup) {
  const interview = state.interviews.find((i) => i.id === interviewId)
  if (!interview) return null

  const place = state.places.find((p) => p.id === interview.placeId) || null
  const center = state.centers.find((c) => c.id === interview.centerId) || null
  ...
  const existingCards = state.cards
    .filter(
      (c) =>
        c.status === 'published' &&
        c.deletedAt === null &&
        (interview.placeId ? c.placeId === interview.placeId : c.centerId === interview.centerId)
    )
    .map((c) => ({ id: c.id, title: c.title, reason: c.reason, category: c.category }))

  const sessionCards = state.cards
  ...
  return {
    sttText,
    questionText,
    place: place ? { id: place.id, name: place.name, category: place.category } : null,
    freightCenter: center ? { id: center.id, name: center.name } : null,
    existingCards,
    sessionCards,
...
// 이번 인터뷰의 draft 카드 조회 (S-06 확인·게시 단계 전용 — 게시 전에는 다른 화면에 노출하지 않는다)
export function getDraftCards(state, interviewId) {
  return state.cards
    .filter((c) => c.interviewId === interviewId && c.status === 'draft' && c.deletedAt === null)
    .map((c) => ({ ...c }))
}
